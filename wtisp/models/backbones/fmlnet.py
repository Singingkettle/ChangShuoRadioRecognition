...
@BACKBONES.register_module()
class FMLNetV47(nn.Module):

    def __init__(self, dropout_rate=0.5, in_features=4,
                 channel_mode=False, skip_connection=False,
                 reduction=16, avg_pool=None):
        super(FMLNetV47, self).__init__()
        self.skip_connection = skip_connection
        # For low snr
        if channel_mode:
            self.conv_net = nn.Sequential(
...
@BACKBONES.register_module()
class FMLNetV48(nn.Module):

    def __init__(self, dropout_rate=0.5, in_features=4,
                 channel_mode=False, skip_connection=False,
                 reduction=16, avg_pool=None):
        super(FMLNetV48, self).__init__()
        self.skip_connection = skip_connection
        # For low snr
        if channel_mode:
            self.conv_net = nn.Sequential(
...
@BACKBONES.register_module()
class FMLNetV49(nn.Module):

    def __init__(self, dropout_rate=0.5, in_features=4,
                 channel_mode=False, skip_connection=False,
                 reduction=16, avg_pool=None):
        super(FMLNetV49, self).__init__()
        self.skip_connection = skip_connection
        # For low snr
        if channel_mode:
            self.conv_net = nn.Sequential(
        ...
        self.gru = nn.GRU(input_size=80, hidden_size=40,
                          num_layers=2, dropout=dropout_rate,
                          batch_first=True, bidirectional=True)

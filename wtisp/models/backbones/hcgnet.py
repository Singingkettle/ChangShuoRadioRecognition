...
class HCGNetV1(nn.Module):

    def __init__(self):
        super(HCGNetV1, self).__init__()
        # For low snr
...
@BACKBONES.register_module()
class HCGNetV3(nn.Module):

    def __init__(self):
        super(HCGNetV3, self).__init__()
        # For low snr
        self.conv_net = nn.Sequential(
            nn.Conv2d(2, 256, kernel_size=(1, 3)),
            nn.ReLU(inplace=True),
        ...
        self.gru1 = nn.GRU(input_size=80, hidden_size=80, batch_first=True)
        self.dropout = nn.Dropout(0.5)
        self.gru2 = nn.GRU(input_size=80, hidden_size=80, batch_first=True)
...
class HCGNetV4(nn.Module):
    ...
    def __init__(self, input_size=80, avg_pool=None, has_stride=False):
        super(HCGNetV4, self).__init__()
        if has_stride:
            stride = 2
        else:
            stride = 1
    ...
    def forward(self, x):
        ...
        c_fea = torch.transpose(c_fea, 1, 2)
        g_fea1, _ = self.gru1(c_fea)
        g_fea2, _ = self.gru2(g_fea1)

        return dict(cnn=torch.sum(c_fea, dim=1), gru1=torch.sum(g_fea1, dim=1), gru2=torch.sum(g_fea2, dim=1))


@BACKBONES.register_module()
class HCGNetV5(nn.Module):

    def __init__(self, input_size=80, avg_pool=None, has_stride=False):
        super(HCGNetV5, self).__init__()
        if has_stride:
            stride = 2
        else:
            stride = 1
        self.conv_net = nn.Sequential(
            nn.Conv2d(4, 256, kernel_size=(1, 3), stride=(1, stride)),
            nn.ReLU(inplace=True),
            nn.Dropout(0.5),
            nn.Conv2d(256, 256, kernel_size=(1, 3), stride=(1, stride)),
            nn.ReLU(inplace=True),

...
class DTN(BaseAMC):
    ...
    def init_weights(self, pre_trained=None):
        ...
        Args:
            pre_trained (str, optional): Path to pre-trained weights.
                Defaults to None.
        """
        super(DTN, self).init_weights(pre_trained)
    ...
    def forward_train(self, iqs, aps, cos, mod_labels, teacher_labels):
        if self.is_iq:
            x = self.extract_feat(iqs)
        else:
            x = self.extract_feat(aps)
        losses = self.classifier_head.forward_train(x, mod_labels=mod_labels, teacher_labels=teacher_labels)

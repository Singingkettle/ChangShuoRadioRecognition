...
class FBSlotDataset(object):
        ...
        if self.data_root is not None:
            if not osp.isabs(self.ann_file):
                ...
                if self.use_cache_data:
                    ...
                    self.iq_data = []
                    self.load_cache_data(cache_data_path + '.pkl')
    ...
    def load_cache_data(self, cache_data_path=None):
        ...
        if os.path.isfile(cache_data_path):
            self.seq_data = pickle.load(open(cache_data_path, 'rb'))
        else:
    ...
    def load_cum(self, data_type, file_name, idx=None):
        if self.use_cache_data:
            seq_data = self.seq_data[idx]['iqs']
...
if __name__ == '__main__':
    a = np.ones((2, 128))
    FBSlotDataset.generate_cumulants(a)

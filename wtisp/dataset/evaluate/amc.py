from ..builder import EVALUATIONS, MERGES
from ..utils import get_classification_accuracy_f1, reshape_results
...
class EvaluateSingleModulationPrediction:
    ...
    def __call__(self, results, data_infos):
        ...
        eval_results = get_classification_accuracy_f1(snr_num, mod_label_num, snr_to_index,
                                                      item_snr_index, results, item_mod_label, prefix=self.prediction_name + '/')
...
class EvaluateMultiModulationPrediction:
    ...
    def __call__(self, results, data_infos):
        ...
        if self.merge:
            selected_results = self.merge(selected_results, data_infos)

        for pr_name in selected_results:
            sub_results = reshape_results(selected_results[pr_name], mod_label_num)
            sub_eval_results = get_classification_accuracy_f1(snr_num, mod_label_num, snr_to_index, item_snr_index,
                                                              sub_results, item_mod_label, prefix=pr_name + '/')
...
class EvaluateSNRPrediction:
    def __init__(self, prediction_name='snr'):
        self.type = 'evaluate-snr-prediction'
        self.prediction_name = prediction_name
    ...
    def __call__(self, results, data_infos):
        ...
        eval_results = get_classification_accuracy_f1(snr_num, snr_label_num, snr_to_index, item_snr_index,
                                                      results, item_snr_label, prefix=self.prediction_name + '/')

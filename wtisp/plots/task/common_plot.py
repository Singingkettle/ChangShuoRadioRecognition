...
class CommonPlot(object):
    def __init__(self, log_dir, config, legend, scatter=None, confusion_map=None,
                 train_test_curve=None, snr_modulation=None,
                 summary=None, vis_fea=None, flops=None, clean_old_version=True,
        ...
        if vis_fea is not None:
            if isinstance(vis_fea, dict):
                self.plot_blocks.append(build_vis_features(vis_fea, scatter_config=self.scatter_config))
            elif isinstance(vis_fea, list):
                self.plot_blocks.extend(build_vis_features(vis_fea, scatter_config=self.scatter_config))
            else:
                raise ValueError('The vis features must be a list or dict!')

        if flops is not None:
            self.plot_blocks.append(build_flops(flops))

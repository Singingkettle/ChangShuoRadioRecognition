...
LOSS_TYPE = Union[torch.Tensor, np.ndarray, Sequence]
...
def format_loss(value: LOSS_TYPE) -> torch.Tensor:
    """Convert various python types to loss-format tensor.
    ...
    if isinstance(value, np.ndarray):
        ...
        raise TypeError(f'Type {type(value)} is not an available loss type.')
    assert value.ndim == 1, \
        f'The dims of value should be 1, but got {value.ndim}.'
...
def batch_label_to_onehot(batch_label, split_indices, num_classes):
    ...
    Examples:
        >>> import torch
        >>> from csrr.structures import batch_label_to_onehot
...
def label_to_onehot(label: LABEL_TYPE, num_classes: int):
    ...
    Examples:
        >>> import torch
        >>> from csrr.structures import label_to_onehot
        >>> # Single-label

...
class ClassificationMetrics:
    """Calculate different performance metrics for classification,
    which are only legal for the task of one class for one data."""

    def __init__(self, pps: np.ndarray, gts: np.ndarray):
        """
        Args:
        ...
        # Ensure the calculation process for a specified metric is only called once
        self._cm = None
    ...
    def _f1_score(self, confusion_matrix) -> np.ndarray:
        f1_score = np.zeros((self.num_class,), dtype=np.float64)
...
class ClassificationMetricsWithSNR(ClassificationMetrics):
    def __init__(self, pps: np.ndarray, gts: np.ndarray, snrs: np.ndarray):
        """
        Args:
        ...
        """
        super(ClassificationMetricsWithSNR, self).__init__(pps, gts)
        self.snrs = snrs

        snr_to_index = {snr: index for index, snr in enumerate(np.sort(np.unique(snrs)))}
    ...
    def confusion_matrix(self) -> np.ndarray:
        if not self._cache_cm:
            confusion_matrix = np.zeros((self.num_snr + 1, self.num_class + 1, self.num_class + 1), dtype=np.float64)
            for snr in self.snr_to_index:
                snr_index = self.snr_to_index[snr]
                confusion_matrix[snr_index, :, :] = self._confusion_matrix(self.pps_along_snr[snr],
                                                                           self.tps_along_snr[snr])
            # Add all snr together
            confusion_matrix[-1, :, :] = np.sum(confusion_matrix[:-1, :, :], axis=0)
    ...
    @property
    def f1_score(self) -> Dict[str, np.ndarray]:
        if not self._cache_fs:
            f1_score = dict()
            confusion_matrix = self.confusion_matrix
            for snr in self.snr_to_index:

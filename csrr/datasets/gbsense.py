...
class GBSenseAdvanced(GBSenseBasic):
    ...
    def evaluate(self, results, logger=None):
        ...
        def _eval(res):
            ...
            for index in sorted_index[-2:]:
                if max_scores[index] >= mod_threshold:
                    y_[index] = max_scores_index[index] + 1
                    selected_number += 1

            if selected_number == 0:
                y_[sorted_index[-1]] = max_scores_index[sorted_index[-1]] + 1
            return y_
...
# @DATASETS.register_module()
# class GBSenseAdvancedB(GBSenseBasic):
#     def __init__(self, file_name, grid_step=0.01, data_root=None, head=None, test_mode=False, is_con=False):
#         super(GBSenseAdvancedB, self).__init__(file_name, grid_step, data_root, test_mode)

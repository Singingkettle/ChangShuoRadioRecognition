...
from .utils import load_amc_evaluation_results
...
def get_new_fig(fn, fig_size=None):
    ...
    ax1.cla()  # clear existing performance
    return fig1, ax1
...
class ConfusionMap(object):
    def __init__(self, log_dir, name, method):
        ...
        self.evaluation_result = load_amc_evaluation_results(self)[0]

    def _evaluate(self, save_dir):
        confusion_matrix = self.evaluation_result['confusion_matrix']
        SNRS = self.evaluation_result['SNRS']
        CLASSES = self.evaluation_result['CLASSES']
        method_name = self.evaluation_result['method_name']
        fig_size = [int(len(CLASSES) / 11 * 7), int(len(CLASSES) / 11 * 7)]
        for snr_index, snr in enumerate(SNRS):
            conf = confusion_matrix[snr_index, :, :]

            save_path = os.path.join(save_dir, method_name + '_snr-' + str(int(snr)) + '_' + self.name)
        ...
        save_path = os.path.join(save_dir, method_name + '_' + self.name)

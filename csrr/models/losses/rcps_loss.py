...
@MODELS.register_module()
class GroupMomentRCPSLoss(nn.Module):
    """Hard CE with reliability-conditioned group posterior moment constraint.

    Labels remain hard at the sample level, while the mean prediction of each
    class-by-reliability group is constrained toward a validation-estimated
    posterior base.
    """
    ...
    def __init__(self,
                 ...
                 epsilon: Optional[Dict] = None,
                 base: Optional[Dict] = None,
                 group_bins: Optional[Sequence[float]] = None,
        ...
        self.reliability_key = reliability_key
        self.reliability_map = reliability_map or dict(type='identity')
        self.epsilon = epsilon or dict(type='constant', value=0.0)
        self.base = base or dict(type='uniform')
        ...
        if self.group_bins is None and 'source' in self.base:
            payload = _load_array(self.base['source'])
            if isinstance(payload, dict) and 'bins' in payload:
                self.group_bins = payload['bins']
        if self.group_bins is None:
            raise ValueError('GroupMomentRCPSLoss requires group_bins or base source with bins.')
    ...
    def forward(self,
                ...
                **kwargs) -> torch.Tensor:
        if data_samples is None:
            raise ValueError('GroupMomentRCPSLoss requires data_samples.')
        if label.ndim != 1:
            raise ValueError('GroupMomentRCPSLoss expects hard class labels, not gt_score.')
        ...
        if self.class_weight is not None:
            class_weight = cls_score.new_tensor(self.class_weight)
        ce = F.cross_entropy(cls_score, label, weight=class_weight, reduction='none')

        gm_weight = build_sample_weight(reliability, self.sample_weight)
        if weight is not None and gm_weight is not None:
            ce_weight = weight.to(cls_score.device).float() * gm_weight
        elif gm_weight is not None:
            ce_weight = gm_weight
        else:
            ce_weight = weight.to(cls_score.device).float() if weight is not None else None
        reduction = reduction_override if reduction_override else self.reduction
        ce_loss = weight_reduce_loss(ce, weight=ce_weight, reduction=reduction, avg_factor=avg_factor)

        eps = build_epsilon(reliability, self.epsilon).reshape(-1)
        if self.constraint_weight <= 0.0 or torch.all(eps <= 0):
        ...
        bins = torch.as_tensor(self.group_bins, dtype=reliability.dtype, device=cls_score.device).flatten()
        if bins.numel() < 1:
            raise ValueError('GroupMomentRCPSLoss group_bins must be non-empty.')
        bin_idx = torch.abs(reliability.reshape(-1, 1) - bins.reshape(1, -1)).argmin(dim=1)
        group_id = label * bins.numel() + bin_idx
        ...
        for gid in torch.unique(group_id):
            ...
            if float(gate.detach().cpu()) <= 0.0:
                continue
            target = _normalize_distribution(base_dist[mask].mean(dim=0, keepdim=True)).squeeze(0)
            pred = _normalize_distribution(prob[mask].mean(dim=0, keepdim=True)).squeeze(0)
            kl = (target * (target.clamp_min(1e-12).log() - pred.clamp_min(1e-12).log())).sum()
            group_terms.append(gate * kl)
        if not group_terms:
            return self.loss_weight * ce_loss
        group_loss = torch.stack(group_terms).mean()
        return self.loss_weight * (ce_loss + self.constraint_weight * group_loss)

...
class FastMLDNNHead(BaseModule):
    """Classification head.
    ...
    def __init__(self,
                 loss: dict = dict(type='CrossEntropyLoss', loss_weight=0.1),
                 beta: float = 0.5,
                 topk: Union[int, Tuple[int]] = (1,),
                 cal_acc: bool = False,
                 init_cfg: Optional[dict] = None):
        super(FastMLDNNHead, self).__init__(init_cfg=init_cfg)

        self.topk = topk
        if not isinstance(loss, nn.Module):
            loss = MODELS.build(loss)
        loss_center = MODELS.build(dict(type='CrossEntropyLoss', loss_weight=beta))
        self.loss_cls = loss
    ...
    def loss(self, feats: Tuple[torch.Tensor], data_samples: List[DataSample],
        ...
        losses = self._get_loss(feats[0], feats[1], data_samples, **kwargs)
        return losses

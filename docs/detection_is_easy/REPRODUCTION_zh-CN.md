...
`tools/detection_is_easy/collect_repro_results.py` 对照
`docs/detection_is_easy/paper_values.csv` 机械生成，该文件把每个报告值存在
它所属的表或章节旁边。
...
`docs/detection_is_easy/paper_values.csv`；用下面的命令重生成对比：

...
战役工具在 [`tools/detection_is_easy/`](../../tools/detection_is_easy)。

## 方法简述

宽带频谱感知被写成 STFT 谱图上的目标检测。两件事驱动全文。第一，定位已经饱和：
视觉检测器的类无关框 mAP 约 0.948 — 找到信号很容易。第二，细粒度识别才是缺口：
57 类的类感知 mAP 只有约 0.45，因为谱图没有用好携带调制身份的相位。论文沿输入
表示、相位效用、检测器复杂度与检测器家族消融纯视觉配方，再加一条领域匹配的
回到 IQ 分支：标成星座家族（PSK/ASK/QAM）的框被信道化回基带 IQ，再用一维层次
识别器重分类，部署 mAP 提高 +0.024 — 起决定作用的是识别器的训练预算，不是架构。
...
结构上限发表（干净准确率 0.643，对上面配方的 0.869）。那条命令里的五个开关
才是发现。
...
尺寸扫描：medium/own 这一格是扫描里最好的单次运行（0.521），而它的 3 种子均值
是 0.477 ± 0.039。论文两个都报，散布也是复杂度结论落在 uniform 列上的原因。

定位行：论文把类无关定位写成 0.948。那个值测在更早、更容易的生成器配置上
（信号时长为观测的 5–100%，SNR −10 到 +20 dB，无同信道重叠），而不是这里发布的
hardshort-lowsnr 基准（时长 0.5–25%，SNR −20 到 +10 dB，35% 同信道重叠）。
在本基准上，匹配的 STFT3 单类运行大约到 0.915，任何配方的最好结果是 0.945。
结论不变 — 定位无论哪种都已饱和，Fig. 2 的定位召回 ≈0.99 也是在本基准测试划分
上测的 — 但跑上面的命令时期望 ≈0.915，而不是 0.948。
...
- **`coco/bbox_mAP`** — mmdet 的 `CocoMetric`，对 57 类平均，在 **val** 划分上。
  这是检测器消融指标：0.447、0.521、0.476，以及上表其余每一格。
- **`class_aware_detection_map`** — `iqdet_metrics.py` 里的时频 IoU 指标，
  在前 2963 个 **test** 场景上计算。这是部署指标：0.522 → 0.546。

部署基线（0.522）与检测器 val mAP（0.521）接近只是巧合。它们是不同划分上的
不同指标。

## 什么算成功复现

训练**不是**确定性的：工具设置 `randomness = dict(seed=..., deterministic=False)`，
所以 cuDNN 会选非确定性核。即使同一种子、同一机器，检测器重跑大约落在
±0.02 类感知 mAP 内；跨种子散布更大（上面尺寸扫描格带着测得的 2–3 种子标准差）。

复现的是稳健的*结论*，而不是第三位小数：

1. 类无关定位 ≈ 0.95，而 57 类 mAP ≈ 0.45。这个缺口就是论文。
2. 仅幅度与相位+幅度打平。谁也不比种子散布赢更多。
3. 可学习前端输给冻结前端。
4. 固定配方下，mAP 从 tiny → large 是平的。
5. 把 PSK/ASK/QAM 框路由回 IQ，总体约 +0.02，PSK 上约 +0.1。
6. 识别器的 120-epoch 配方比 40-epoch 前任高出约 +0.23 干净准确率。
   这是预算效应，不是架构效应。

若 (1)–(6) 成立，即使个别格子在第二位小数上不同，复现也算成功。
...
- **定位数字。** 见复现表下的提醒：论文的 0.948 来自更早、更容易的生成器配置；
  本基准上匹配运行约为 0.915。

...
# the scene used in the paper: the first with 3-4 signals. It carries exactly one genuine
# label error (am-lsb read as am-usb -- a sideband confusion), which is the point of the figure.
pick = next(iid for iid, a in by_img.items() if 3 <= len(a) <= 4)
...
for p in sorted(preds_by_img.get(pick, []), key=lambda d: -d["score"])[:len(gts)]:
    ...
    ax.add_patch(mpatches.Rectangle((x, y), w, h, fill=False, edgecolor=PR_C, lw=LW_PR,
                                    ls=(0, (4, 2))))
    # bottom-edge boxes get their label above-right, so the label box never clips
    if y + h > 0.80 * H:

...
# Runs the campaign named separately that are the same cell, or that the paper pooled.
# A run may land in more than one: the deployment detector is its own row *and* one of
# the three seeds the paper averaged for medium/own.
ALSO_IN = {
    "floor_m_uniform": ["bu_m_uniform"],
    "floor_tiny_uniform": ["bu_tiny_uniform"],
    "deploy_m_own": ["bo_m_own"],
}
...
def write_markdown(path: Path, pooled: list[dict], runs: list[dict], tolerance: float) -> None:
    lines = [
        ...
        "",
        "| cell | seeds | reproduced | sd | paper | delta | verdict | paper source |",
        "|---|---|---|---|---|---|---|---|",
    ]
    for row in pooled:
        ...
        lines.append(
            ...
            f"{paper} | {delta} | {verdict(row, tolerance)} | {row['paper_source'] or '--'} |"
        )

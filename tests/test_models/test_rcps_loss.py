...
from csrr.models.losses import (LabelSmoothingCrossEntropyLoss,
                                GroupMomentRCPSLoss, RCPSCrossEntropyLoss,
                                build_rcps_targets)
...
def test_group_moment_epsilon_zero_matches_hard_ce():
    logits = torch.tensor([[2.0, 0.0, -1.0],
                           [0.0, 1.5, -0.5],
                           [0.2, 0.1, 1.0],
                           [0.3, 1.1, 0.0]])
    labels = torch.tensor([0, 1, 2, 1])
    samples = [_sample(0, -20), _sample(1, -20), _sample(2, 18), _sample(1, 18)]
    loss = GroupMomentRCPSLoss(
        reliability_map=dict(type='linear', min=-20, max=18),
        epsilon=dict(type='constant', value=0.0),
        group_bins=[0.0, 1.0],
        base=dict(type='uniform'),
...
def test_group_moment_requires_group_bins():
    with pytest.raises(ValueError):
        GroupMomentRCPSLoss(base=dict(type='uniform'))

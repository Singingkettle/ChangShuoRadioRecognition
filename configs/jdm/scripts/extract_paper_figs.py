...
points at configs/jdm/paper_figure_targets.md.
...
def _repo_root() -> Path:
    for parent in Path(__file__).resolve().parents:
        if (parent / 'tools' / 'train.py').is_file() and (parent / 'csrr').is_dir():
            return parent
    raise RuntimeError('cannot locate repository root from this script')
_REPO = _repo_root()
_TARGETS = _REPO / "configs/jdm/paper_figure_targets.md"

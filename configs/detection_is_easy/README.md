...
**Reproduction level: `statistical`.** The public pipeline preserves the protocol and
generator settings, but it cannot regenerate the paper's exact corpus realization.
...
that produced the deployment tie the paper nearly published as a structural limit (combined
clean accuracy 0.534 vs 0.875 for the recipe above; the pair was originally reported as 0.643
vs 0.869, which mixed a stage2-single number with a combined one). The five flags in that
command are the finding.
...
**How to read "expected".** Every detector expectation is a **3-seed mean ± sd** from the
end-to-end re-run recorded in [`REPRODUCTION.md`](REPRODUCTION.md), and it is also what the
manuscript now reports; a single run lands within ±0.023 of it (three times the measured
same-seed floor). Cells the paper states from one seed are listed as single values. The last
column keeps the value the paper *originally* reported, so the history is not lost; where it
differs, `REPRODUCTION.md` says why.

| paper cell | config | recipe / flags | seeds | expected (3-seed mean ± sd) | tol. | originally reported |
|---|---|---|---|---|---|---|
| Localization is easy (class-agnostic) | `rtmdet_m_stft3_tensor_memmap_resize512.py` | uniform, `--batch-size 8`, **`--root $MM/coco`** | any, 3 seeds | 0.893 ± 0.006 | ±0.023 | 0.948 (easier generator configuration) |
...
| Tab. I medium / uniform (= Tab. III STFT3 offline reference) | `rtmdet_m_stft3_tensor_memmap_resize512.py` | uniform, `--batch-size 8` | any, 3 seeds | 0.472 ± 0.012 | ±0.023 | 0.460 (the Tab. III reference cell was added in revision) |
| Tab. I large / uniform | `rtmdet_l_stft3_tensor_memmap_resize512.py` | uniform, `--batch-size 8` | any, 3 seeds | 0.451 ± 0.007 | ±0.023 | 0.462 |
| Tab. I tiny / own | `rtmdet_tiny_stft3_tensor_memmap_resize512.py` | own, `--batch-size 4` | any, 3 seeds | 0.433 ± 0.004 | ±0.023 | 0.408 (2 seeds) |
| Tab. I small / own | `rtmdet_s_stft3_tensor_memmap_resize512.py` | own, `--batch-size 4` | any, 3 seeds | 0.470 ± 0.014 | ±0.023 | 0.429 |
| Tab. I medium / own **(deployment detector)** | `rtmdet_m_stft3_tensor_memmap_resize512.py` | own, `--batch-size 4` | any, 3 seeds | 0.492 ± 0.010 | ±0.023 | 0.477 ± 0.039 (pooled with the batch-8 run, 0.521) |
| Tab. I large / own | `rtmdet_l_stft3_tensor_memmap_resize512.py` | own, `--batch-size 4` | any, 3 seeds | 0.504 ± 0.015 | ±0.023 | 0.486 (2 seeds) |
...
| Tab. III phase + magnitude | `rtmdet_m_raw_iq_filterbank_hardshort_resize512.py` | uniform, `--batch-size 6` | any, 3 seeds | 0.455 ± 0.013 | ±0.023 | 0.447 (2 seeds) |
...
| §VI-B FCOS | `fcos_stft3_memmap_resize512.py` | uniform, `--batch-size 4`, **`--require-tensor-stats`** | any, 3 seeds | 0.470 ± 0.005 | ±0.023 | 0.374 (identity normalisation, first round) |
| §VI-B ATSS | `atss_stft3_memmap_resize512.py` | uniform, `--batch-size 4`, **`--require-tensor-stats`** | any, 3 seeds | 0.468 ± 0.003 | ±0.023 | 0.380 (identity normalisation, first round) |
| §VI-B RTMDet-M, matched batch | `rtmdet_m_stft3_tensor_memmap_resize512.py` | uniform, `--batch-size 4` | any, 3 seeds | 0.426 ± 0.001 | ±0.023 | — (added in revision) |
...
| §VI-D recognizer, 40-epoch predecessor | — | `train-hier` **defaults** (`--epochs 40 --aug-cfo 0.02`) | 101 | 0.632 stage2-single / 0.534 combined | ±0.01 | 0.643 (stage2-single, labelled as combined) |
...
The deployment detector and its batch size: the Stage-1 command above trains at
`--batch-size 8`, which is the configuration every bridged number in this README was computed
from. Its single run was originally reported as 0.521 and listed here as the medium/own cell;
the reproduction of that single run gives 0.472, and the manuscript no longer cites it. The
medium/own cell is now stated, like the rest of the own-schedule column, at `--batch-size 4`
over three seeds (0.492 ± 0.010); the originally reported 3-seed mean, 0.477 ± 0.039, had
pooled the batch-8 run with two batch-4 runs. The complexity conclusion still rests on the
uniform column.

The localization row: the paper originally quoted 0.948 for class-agnostic localization. That
value was measured on an earlier, easier generator configuration (signal durations 5–100 % of
the observation, SNR −10 to +20 dB, no co-channel overlap) rather than on the hardshort-lowsnr
benchmark released here (durations 0.5–25 %, SNR −20 to +10 dB, 35 % co-channel overlap). On
this benchmark three single-class runs give 0.893 ± 0.006, and that is the value the
manuscript now reports (an earlier revision of this README put the single-run expectation at
≈0.915). The conclusion is unchanged — localization is saturated either way, and Fig. 2's
localization recall of ≈0.99 is measured on this benchmark's test split — but expect ≈0.89,
not 0.948, when you run the command above.

The FCOS/ATSS rows: the first-round values 0.374 / 0.380 were trained with identity
normalisation (`mean=[0,0,0] std=[1,1,1]` on data whose per-channel sigma is about 12.8),
while the RTMDet runs they were compared against used the measured statistics. With the
statistics injected (`--require-tensor-stats`) they reach 0.470 ± 0.005 / 0.468 ± 0.003,
above RTMDet-M at the same batch (0.426 ± 0.001); the manuscript reports these, and
0.374 / 0.380 survive only as history.
...
- **`coco/bbox_mAP`** — mmdet's `CocoMetric`, averaged over the 57 categories, on the **val**
  split. This is the detector ablation metric: 0.472, 0.492, 0.441, and every other cell in the
  table above.
- **`class_aware_detection_map`** — the time-frequency IoU metric in `iqdet_metrics.py`,
  computed over the first 2963 **test** scenes. This is the deployment metric: the +0.028
  fused delta over a reproduced vision baseline of 0.474 (originally reported as
  0.522 → 0.546).

The deployment baseline (reproduced 0.474; originally 0.522) and the val mAP of the detector
it is bridged from (reproduced 0.472 for that run; originally 0.521) are close by
coincidence. They are different metrics on different splits.
...
detector re-run lands within the ±0.023 band class-aware mAP; across seeds the spread is
larger (the cells above carry their measured three-seed standard deviations).

Reproduce the *conclusions*, which are robust, rather than chasing the third decimal:

1. Class-agnostic localization ≈ 0.89 while 57-class mAP ≈ 0.45. The gap is the paper.
...
5. Routing PSK/ASK/QAM boxes back to IQ gains ≈ +0.03 overall and ≈ +0.15 on PSK with a
   recognizer trained on ground-truth boxes, and ≈ +0.09 overall once the recognizer is
   trained on the detector's own predicted boxes.
6. The recognizer's 120-epoch recipe beats its 40-epoch predecessor by ≈ +0.34 combined clean
   accuracy (0.534 → 0.875; ≈ +0.24 on the stage2-single metric, 0.632 → 0.867). This is a
   budget effect, not an architecture effect.
7. At matched batch and measured normalisation the detector family does not separate
   (FCOS 0.470, ATSS 0.468, RTMDet-M 0.426).

If (1)–(7) hold, the reproduction succeeded even if individual cells differ in the second
decimal.
...
- **The localization number.** See the caution under the reproduction table: the originally
  reported 0.948 came from an earlier, easier generator configuration; this benchmark gives
  0.893 ± 0.006 over three single-class runs, which is what the manuscript now reports.
- **FCOS/ATSS normalisation.** The first-round FCOS/ATSS values (0.374 / 0.380) were trained
  with identity normalisation; always pass `--require-tensor-stats` for these heads. With it
  they reach 0.470 ± 0.005 / 0.468 ± 0.003, the values the manuscript reports.

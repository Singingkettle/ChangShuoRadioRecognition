...
**读 `paper` 各列时请记住一件事。** 稿件此后已按这次重跑订正：`paper_values.csv`
里的论文值，以及下面每张表的 `paper (current)` 列，都是稿件的*当前*数字，而它们
正是依据本复现订正的。因此 `reproduced` 与 `paper (current)` 一致并不是独立验证 —
从构造上就是循环的。为此每张表都另带一列 `originally reported`，即最初投稿时
稿件里的值；“订正了什么、为什么” 一节逐项记录每处更改及其原因。`reproduced`
各列是未经改动的测量值。
...
`paper (current)` 是稿件当前值（三种子格给出均值 +- sd）；`originally reported`
是最初投稿时稿件里的值，种子数不同时一并注明；`delta` 与 “verdict” 是
`reproduced` 减 `paper (current)`，对照 0.023 带。
...
| Axis B, tiny, own sched. | 0.433 | 0.0035 | 0.433 +- 0.004 | 0.408（2 种子） | 0.000 | within | Table I own |
| Axis B, small, own sched. | 0.470 | 0.0135 | 0.470 +- 0.014 | 0.429 | 0.000 | within | Table I own |
| Axis B, medium, own sched. | 0.486 | 0.0152 | 0.492 +- 0.010（batch 4） | 0.477 +- 0.039（混入 batch-8 运行） | -0.006 | within | Table I own |
| Axis B, large, own sched. | 0.504 | 0.0154 | 0.504 +- 0.015 | 0.486（2 种子） | 0.000 | within | Table I own |
| Axis A, STFT3 offline (reference) | 0.472 | 0.0121 | 0.472 +- 0.012 | --（首次投稿无此格） | 0.000 | within | Table III；按构造与 Axis B medium/uniform 是同一组运行 |
| Axis A, magnitude-only | 0.441 | 0.0231 | 0.441 +- 0.023 | 0.455 | 0.000 | within | Table III |
| Axis A, phase+magnitude | 0.455 | 0.0125 | 0.455 +- 0.013 | 0.447（2 种子） | 0.000 | within | Table III |
...
| FCOS | 0.470 | 0.0053 | 0.470 +- 0.005 | 0.374（恒等归一化，1 种子） | 0.000 | within | Table II (Axis C), §VI-B |
| ATSS | 0.468 | 0.0032 | 0.468 +- 0.003 | 0.380（恒等归一化，1 种子） | 0.000 | within | Table II (Axis C), §VI-B |
| RTMDet-M, uniform, batch 4 | 0.426 | 0.0006 | 0.426 +- 0.001 | --（首次投稿无此格） | 0.000 | within | Table II (Axis C), §VI-B, matched-batch control |
| localization (single-class) | 0.893 | 0.0060 | 0.893 +- 0.006 | 0.948（更容易的生成器配置） | 0.000 | within | Sections I, IV, VI-A |

`deploy_m_own` 这一格 — 部署检测器的单次最好运行，最初报告为 0.521 — 不再出现在
本表，因为稿件已不再引用该数字；那次单次运行的复现值（0.472）以及这个数字的去向
记在 “订正了什么、为什么” 一节。
...
|---|---|---|---|
| RTMDet-M, own sched., batch 4 | 0.491 | -- | batch-decoupled size control (§VI-C, L5)；稿件当前的 medium/own 值（0.492 +- 0.010）就是这个 batch-4 配置的三种子结果 |

带当前论文值的 18 格全部落在带内。按本文开头的披露，这是订正本身保证的结果，
不是独立证据。若对照*最初报告值*，有 7 格不在带内 — tiny/own、small/own、
complex-1D + FFT、FCOS、ATSS、定位，以及部署检测器的单次最好运行 — 它们每一格
此后都已在稿件中订正。各格连同被替换的值，在下面 “订正了什么、为什么” 一节逐项
交代。
...
| 40-epoch predecessor | stage2-single acc | 0.632 | 0.632 | 0.643（被标成 combined） | run-index 20 -- see dose-response note |
| 40-epoch predecessor | combined fine acc | 0.534 | 0.534 | --（未报告） | |
...
这里的 `paper (current)` 列同样适用上面的提醒：它们是订正后的稿件数字，而且是依据
这次重跑定下来的。
...
每一条都写明最初报告值、重跑测到的值，以及稿件现在怎么说。标题里的差值是
`reproduced` 减 `originally reported`，即触发订正的那个偏差。

**FCOS +0.096，ATSS +0.088（§VI-B）— 最初报告 0.374 / 0.380，订正为
0.470 +- 0.005 / 0.468 +- 0.003。** 最初报告的 FCOS/ATSS 运行曾用恒等归一化训练 —
生成配置带着 `mean=[0,0,0] std=[1,1,1]`，而数据的逐通道 sigma 大约 12.8 —
它们对照的 RTMDet 运行却用了真实统计。用注入统计重跑 FCOS/ATSS
（`--require-tensor-stats`）得到了 0.470 与 0.468，三种子 sd 为 0.005 与 0.003。
另外两件事把图景收得更紧：
...
- 所以最初报告的 RTMDet 领先来自两个叠加伪影：归一化缺陷拖累其他头，以及 batch
  大小不同。两者都去掉后，检测器家族分不开。

稿件已采纳：恒等归一化这一根因现写在 §VI-C，家族对比改为报告匹配 batch 下
FCOS/ATSS 高于 RTMDet-M（并把 batch-4 的 RTMDet-M 对照 0.426 +- 0.001 加为一格），
首轮的 0.374 / 0.380 只作为这里记录的历史保留。这一发现与论文论点一致 — 定位
已饱和，缺口在识别。

**定位 −0.055（§I/IV/VI-A）— 最初报告 0.948，订正为 0.893 +- 0.006。** 最初报告的
类无关定位 mAP 0.948 测在比已发布 `hardshort_lowsnr` 基准更容易的生成器配置上
（`lowsnr` 集：更长时长、更高 SNR、无同信道重叠）。在已发布基准上三次单类运行
给出 **0.893 +- 0.006**。定位仍是容易、接近饱和的轴；稿件现在用在它所分发的
基准上测得的数字来陈述它。

**部署检测器 −0.049 — 最初报告 0.521（单次最好运行），现已不再引用。** 首次投稿把
这一格的单次最好运行（0.521，一次 batch-8 运行）当作部署基线，却把三种子均值
（0.477，把那次 batch-8 运行与两次 batch-4 运行混在一起）用于尺寸扫描。复现给出
单次 0.472，混合三种子均值 0.486。稿件已不再引用 0.521：部署检测器统一表述为
batch 4、三种子的 medium/own 格，0.492 +- 0.010（复现的 batch-4 对照为 0.491），
`deploy_m_own` 这一格也从上表删去。部署数字都从部署 checkpoint 桥过去，所以绝对
水平跟着它走（复现基线 0.474 对最初报告的 0.522）；*增量*无论怎样都复现了，并已
按识别器表所列刷新。

**small/own +0.041，tiny/own +0.025 — own-schedule 列最初报告 0.408 / 0.429 /
0.477 / 0.486，订正为 0.433 / 0.470 / 0.492 / 0.504。** own-schedule 列上的真实偏差，
该列最初报告的散布本身就大（small 三种子 sd 0.017，tiny 两种子 sd 0.041），而且
tiny 与 large 两格只有两个种子。稿件现在把整列报告为 lr 1e-4、batch 4、三种子：
tiny / small / medium / large 为 0.433 +- 0.004 / 0.470 +- 0.014 / 0.492 +- 0.010 /
0.504 +- 0.015。同样尺寸的 uniform 列本就干净复现，也一并刷新为重跑均值
（0.431 / 0.449 / 0.460 / 0.462 -> 0.432 / 0.443 / 0.472 / 0.451）。

**complex-1D +0.027 — 最初报告 0.026，订正为 0.053。** 两个值都是塌缩；在那个量级上
差值没有意义。Axis E 的发现 — 先学习再 FFT 的前端会毁掉定位 — 复现了，稿件现在
写的是重跑值。

**Axis A 中本在带内、但也刷新了的格子。** 仅幅度 0.455 -> 0.441 +- 0.023，
相位+幅度 0.447（两种子）-> 0.455 +- 0.013（三种子），仅相位 0.440 -> 0.431，
可学习滤波器组 0.412 -> 0.418。这些都没有改变任何判定：仅幅度与相位+幅度仍在
...
这个配方是按检测器的，而且能推广。在 FCOS 上重复整套流程 — 导出 FCOS 的训练集
预测、构建预测框缓存、在其上训识别器、部署到 FCOS 的测试集预测 — 三种子给出
**+0.189 +- 0.002**（0.469 -> 0.659），比 RTMDet 的更大，因为更弱的视觉头给 IQ
分支留下更多可挽回的空间。训练必须与检测器匹配：在 RTMDet 的框上训练、不经重训
直接部署到 FCOS 框上的识别器只得 +0.040（高于 GT 框 recipe-A 在 FCOS 上的 +0.034，
所以分布部分可迁移），只有在检测器自己的框上训练才能拿回全部增益。
...
分家族看，RTMDet 预测框识别器在同样三个种子上的增益为 PSK +0.453 +- 0.011、
QAM +0.537 +- 0.023、ASK +0.373 +- 0.016
（`work_dirs/repro/deployment/bridge_predhi_s{101,202,303}/summary.csv`）。

**预测框识别器的 PSK 增益 — 曾写为 +0.457，订正为 +0.453 +- 0.011。** 稿件的一个
中间修订版曾把预测框识别器的 PSK 增益写成 +0.457，那只是种子 101 一个值。它已
换成上述 summary 文件给出的三种子均值 +0.453 +- 0.011；QAM（+0.537 +- 0.023）与
ASK（+0.373 +- 0.016）按同样的三种子口径给出。

**剂量响应曲线（§VI-D）— 最初报告 0.643 -> 0.714 -> 0.869，订正为
0.632（stage2-single）/ 0.534（combined）-> 0.699 -> 0.875。** 最初报告的
0.643 -> 0.714 -> 0.869 从三次运行各取了一种不同指标。干净地测，每种指标各自
都是单调的：stage2-single 准确率 0.632 / 0.691 / 0.867，combined fine 准确率
0.534 / 0.699 / 0.875。最初报告的 0.643 是被标成 combined 的 stage2-single 数字。
稿件现在把每种指标都标在各自名下 — 40-epoch 为 0.632 stage2-single 与 0.534
combined，recipe-B 0.699，recipe-A 0.875 — 差分相位识别器为 0.913（最初 0.916），
部署增量刷新为融合 +0.028（最初 +0.024）、psk / ask / qam +0.153 / +0.132 / +0.081
（最初 +0.143 / +0.118 / +0.084）、差分相位 +0.022（最初 +0.019）。oracle 上界
0.608 未变。

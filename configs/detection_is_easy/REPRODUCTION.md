...
| predicted-box recogniser, RTMDet (3 seeds) | fused delta | +0.093 +- 0.001 | +0.093 +- 0.001 | -0.019 (high-score cache) | see "originally reported as -0.019" below |
...
**Predicted-box recogniser (§VII) -- originally reported as -0.019, corrected to +0.093.**
The first submission's negative-table row for "train the recogniser on predicted boxes"
reported a deployment delta of -0.019. The reproduction gave **+0.093**, at three seeds
...
- *Explains the originally reported -0.019.* The first submission's crop cache was named
  ...
  (`--score-thr 0.1`) is what produces the +0.093. The originally published negative result
  was an artefact of building the cache from high-score, near-GT boxes. The manuscript now
  reports the predicted-box recogniser as a positive result at +0.093 +- 0.001 and gives the

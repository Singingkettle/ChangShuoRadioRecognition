...
# Axis-C cross-detector cell: three-stage Cascade R-CNN (R50-FPN) on the SAME STFT3 memmap
# input as the RTMDet cells, 57-class via --root coco_multiclass. A stronger two-stage head
# than Faster R-CNN; another test of predicted-box generalisation. num_classes is left to the
# harness, which recurses over the model dict and sets all three cascade bbox heads (writing
# a dict into roi_head here would clash with the base's list of heads). RoIAlign runs via the
...
load_from = None
resume = False
work_dir = 'work_dirs/torchsig_mmdet_stft3_memmap_cascade_rcnn_r50'
